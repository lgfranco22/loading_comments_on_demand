...
function loadComm() {
    $.ajax({
        ...
        //console.log(res);
        var newArea = area.innerHTML;
        if(res.status == 'success'){
            for(var i = 0; i <= (res.data.length -1); i++){
                newArea += `<div class="comment"><div class="avatar">${res.data[i].name}</div><div class="message">${res.data[i].comment}</div></div>`;
            }
        } 
        else{
            btn.remove();
